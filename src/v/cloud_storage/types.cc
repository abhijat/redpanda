...
void upload_object_request::on_success(remote_probe& probe) {
    do_callback(success_cb, probe);
}
void upload_object_request::on_failure(remote_probe& probe) {
    do_callback(failure_cb, probe);
}
void upload_object_request::on_backoff(remote_probe& probe) {
    do_callback(backoff_cb, probe);
}

void upload_object_request::do_callback(
  std::optional<probe_callback_t>& cb, remote_probe& probe) {
    if (cb.has_value()) {
        cb.value()(probe);
    }
}
